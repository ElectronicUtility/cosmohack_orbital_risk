...
function renderSelected() {
  ...
  $("selected-factors").innerHTML = w.factors
    ...
    .map(
      (f) =>
        `<div class="factor-row"><button data-factor="${esc(f.mechanism)}" data-tip="${esc(factorValue(f) + ". " + eligibility(f))}" aria-label="Сближения: открыть данные"><span>Сближения</span><strong>${Number.isFinite(f.comparison_value) ? esc(factorValue(f)) : "Нет данных"} <span aria-hidden="true">›</span></strong></button></div>`,
    )
    .join("");
  renderTimeline(w);
  setSample(0);
...
async function loadValidation() {
  try {
    ...
    $("validation-summary").innerHTML =
      `<div class="validation-numbers"><span><strong>${d.windows}</strong>окон</span><span><strong>${d.improved_cases}/${d.cases}</strong>улучшено</span><span><strong>${d.worse_cases}</strong>ухудшений</span></div><div class="validation-actions">${infoButton(tip, "О проверке освещения")}<a href="${apiURL("/api/validation")}" target="_blank" rel="noopener">Результаты</a></div>`;
  } catch {
